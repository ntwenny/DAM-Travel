...
const ItemRow = ({
    ...
    return (
        ...
        <View
            className={`flex-row items-center justify-between p-3 my-2 bg-card rounded-lg border border-border`}
        >
    ...
    return (
        <View className="flex-row items-center justify-between py-3">
            <View className="flex-row items-center">
                ...
                <View
                    className={`w-3 h-3 rounded-full mr-2 ${type === "refund" ? "bg-[${COLORS.taxRefundBlue}]" : "bg-[${COLORS.red}]"}`}
                />
...
export default function MockReceiptScreen() {
    ...
    const selectedCategoryOption = useMemo(() => {
        const cat = finance?.categories?.find(
    ...
    useEffect(() => {
        async function fetchReceipt() {
            ...
            const rec = await createReceiptForTrip(
    ...
    // Fetch current trip to get trip currency - use getTrips for full data
    useEffect(() => {
        async function fetchTrip() {
            ...
            try {
                ...
                const trip =
                    trips?.find(
                        (t: Trip) => t.id === userProfile.currentTripId
                    ) || null;
                setCurrentTrip(trip);
            } catch (err) {
    ...
    // Calculate home currency from user profile using country list
    ...
    }, [userProfile?.currentTripId, homeCurrency]);
    ...
    async function handleApplyToBudget() {
        ...
        if (!creatingNewCategory && !selectedCategoryId) {
            Alert.alert(
                "Pick a category",
                'Select a category or choose "Create new category".'
            );
            return;
        }
    ...
    const BestWayToPay = () => (
        <View className="p-3 my-4 bg-primary/30 rounded-lg border border-yellow-200">
            <Text className="text-lg font-semibold">Best Way to Pay:</Text>
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 20 }}>
                ...
                <View className={`pt-12 pb-20 bg-[${COLORS.lightBlue}]`}>
                    <View className="px-5 flex-row items-center justify-between">
                        ...
                        >
                            <Text className="text-lg font-bold text-white pr-1">
                                {getCurrencySymbol(
                                    displayCurrency || currencyLocal
                                )}
                            </Text>
                    ...
                    <Text className="text-[32px] font-bold text-center text-white mt-4">
                        {formatAmount(
                            totalDisplay,
                            displayCurrency || currencyLocal
                        ).replace(String(displayCurrency || currencyLocal), "")}
                    </Text>
                </View>
            ...
            */}
                ...
                <View className="px-4 mt-[-60]">
                    <View className="bg-background p-4 rounded-t-lg shadow-md">
                        ...
                        <Text className="text-[18px] text-gray-500 mb-4">
                            {date}
                        </Text>

                        {/* Dashed line separator (Light Grey) */}
                        <View
                            className={`border-b border-dashed border-border mb-4`}
                        />
                        ...
                        {/* Dashed line separator (Light Grey) */}
                        <View
                            className={`border-b border-dashed border-[${COLORS.lightGrey}] my-4`}
                        />
                        ...
                        {/* Dashed line separator (Light Grey) */}
                        <View
                            className={`border-t border-dashed border-[${COLORS.lightGrey}] mt-6 pt-4`}
                        >
